...
import AnswerPart3 from './AnswerPart3';
...
const Arr3 = ({ exam, part3, index, topics, userAnswerDetail = null }: ArrayPart3Props) => {
  ...
  const getAnswerIcon = () => {
    ...
    return currentUserAnswerDetail.selectedAnswer === part3.correctAnswer 
...
const displayCorrectAnswer = () => {
  return `(${part3.correctAnswer.charAt(1)})`; // Lấy ký tự đầu tiên (ví dụ: (B))
};

const buttonColorClass = currentUserAnswerDetail && currentUserAnswerDetail.selectedAnswer
  ? currentUserAnswerDetail.selectedAnswer === part3.correctAnswer
  ...
  return (
    <div className="flex flex-col justify-start items-start gap-2 flex-nowrap">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {index + 32} {/* Display number from 3 */}
        </Button>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-[900px] w-[90%]">
          <DialogHeader className="px-2">
            <DialogTitle>Thông tin Câu hỏi {index + 32}</DialogTitle>
          </DialogHeader>

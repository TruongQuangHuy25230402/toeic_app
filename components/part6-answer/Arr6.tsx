...
import AnswerPart6 from './AnswerPart6';
...
const Arr6 = ({ exam, part6, index, topics, userAnswerDetail = null }: ArrayPart6Props) => {
  ...
  const getAnswerIcon = () => {
    ...
    return currentUserAnswerDetail.selectedAnswer === part6.correctAnswer 
  ...
  const displayCorrectAnswer = () => {
    return `(${part6.correctAnswer.charAt(1)})`; // Lấy ký tự đầu tiên (ví dụ: (B))
  };

  const buttonColorClass = currentUserAnswerDetail && currentUserAnswerDetail.selectedAnswer
  ? currentUserAnswerDetail.selectedAnswer === part6.correctAnswer
  ...
  return (
    <div className="flex flex-col justify-start items-start gap-2 flex-nowrap">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {index + 131} {/* Display number from 6 */}
        </Button>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-[900px] w-[90%]">
          <DialogHeader className="px-2">
            <DialogTitle>Thông tin Câu hỏi {index + 131}</DialogTitle>
          </DialogHeader>

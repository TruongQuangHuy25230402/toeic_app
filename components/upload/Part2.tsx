"use client";
import { createBulkPart2s, deletePart2s, Part2Props } from "@/actions/getPart2";
import React, { useEffect, useState } from "react";
import * as XLSX from "xlsx";
import { usePathname, useRouter } from "next/navigation"; // Import necessary hooks
import { QuestionPart2 } from "@prisma/client";
import axios from "axios";
import { UploadButton } from "../uploadthing";


const Part2 = ({ part2s }: { part2s: Part2Props[] }) => {
  ...
  const [arrPart2, setArrPart2] = useState<QuestionPart2[]>([]);
    ...
    if (file && examId) {
      ...
      reader.onload = async (e) => {
        ...
        if (data) {
          ...
          const sheetName = workbook.SheetNames[0];
          const workSheet = workbook.Sheets[sheetName];
          const json: Part2Props[] = XLSX.utils.sheet_to_json(workSheet);
          ...
          try {
            await createBulkPart2s(dataWithExamId);
            setLoading(false);
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const response = await axios.get(`/api/part2?examId=${examId}`);
        setArrPart2(response.data.arr); // Cập nhật state với danh sách câu hỏi
      } catch (error) {
    ...
    <div className="py-8 space-y-8">
      ...
      {arrPart2.length === 0 && (
        ...
        <button
          ...
          className="py-2 px-6 rounded bg-blue-600 text-slate-200"
        >
          ...
          {arrPart2.length > 0 ? (
            <div className="relative overflow-x-auto">
              <div>
                <button
                  onClick={clearData}
                  className="py-2 px-6 rounded bg-red-600 text-slate-200"
                >
                  Clear Data
                </button>
              </div>
                  ...
                  {arrPart2.map((part2) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="truncate max-w-xs" title={part2.explainAnswer}>
                          {part2.explainAnswer}
                        </div>
                      </td>
                    </tr>
            ...
            <div>
              <div className="flex items-center gap-8">
                <div>
                  ...
                  <input
                    className="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-20 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                    id="file_input"
                    type="file"
                    accept=".xls,.xlsx"
                ...
                <button
                  onClick={saveData}
                  className="py-2 px-6 rounded bg-purple-600 text-slate-200"
                  disabled={loading}
                >

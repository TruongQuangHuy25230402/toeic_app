...
import { createBulkQuestionss, deleteQuestionss, QuesProps } from "@/actions/getQuestionP";

const Upload = ({ questions }: { questions: QuesProps[] }) => {
  ...
  const [arrQuestions, setArrQuestions] = useState<Ques[]>([]);
    ...
    if (file && examsId) {
      ...
      reader.onload = async (e) => {
        ...
        if (data) {
          ...
          const sheetName = workbook.SheetNames[0];
          const workSheet = workbook.Sheets[sheetName];
          const json: QuesProps[] = XLSX.utils.sheet_to_json(workSheet);
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const response = await axios.get(`/api/ques-list`);
    ...
    fetchQuestions();
  }); // Khi examsId thay đổi, useEffect sẽ chạy lại

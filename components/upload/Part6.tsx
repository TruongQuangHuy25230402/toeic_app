"use client";
import { createBulkPart6s, deletePart6s, Part6Props } from "@/actions/getPart6";
import React, { useEffect, useState } from "react";
import * as XLSX from "xlsx";
import { usePathname, useRouter } from "next/navigation"; // Import necessary hooks
import { QuestionPart6 } from "@prisma/client";
import axios from "axios";

const Part6 = ({ part6s }: { part6s: Part6Props[] }) => {
  ...
  const [arrPart6, setArrPart6] = useState<QuestionPart6[]>([]);
    ...
    if (file && examId) {
      ...
      reader.onload = async (e) => {
        ...
        if (data) {
          ...
          const sheetName = workbook.SheetNames[0];
          const workSheet = workbook.Sheets[sheetName];
          const json: Part6Props[] = XLSX.utils.sheet_to_json(workSheet);
          ...
          try {
            await createBulkPart6s(dataWithExamId);
            setLoading(false);
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const response = await axios.get(`/api/part6?examId=${examId}`);
        setArrPart6(response.data.arr); // Cập nhật state với danh sách câu hỏi
      } catch (error) {
            ...
            <tbody>
              {arrPart6.map((part6) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">{part6.answer1}</td>
                  <td className="px-6 py-4">{part6.answer2}</td>
                  <td className="px-6 py-4">{part6.answer3}</td>
                  <td className="px-6 py-4">{part6.answer4}</td>
                  <td className="px-6 py-4">{part6.correctAnswer}</td>
                  <td className="px-6 py-4">
                    <div
                      className="truncate max-w-xs"
                      title={part6.explainAnswer}
                    >
                      {part6.explainAnswer}
                    </div>
                  </td>
        ...
        <div>
          <div className="flex items-center gap-8">
            <div>
              ...
              <input
                className="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-60 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                id="file_input"
                type="file"
                accept=".xls,.xlsx"
                onChange={(e) =>

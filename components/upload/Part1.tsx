"use client";
import { createBulkPart1s, deletePart1s, Part1Props } from "@/actions/getPart1";
...
const Part1 = ({ part1s }: { part1s: Part1Props[] }) => {
  ...
  const [arrPart1, setArrPart1] = useState<QuestionPart1[]>([]);
    ...
    if (file && examId) {
      ...
      reader.onload = async (e) => {
        ...
        if (data) {
          ...
          const sheetName = workbook.SheetNames[0];
          const workSheet = workbook.Sheets[sheetName];
          const json: Part1Props[] = XLSX.utils.sheet_to_json(workSheet);
          ...
          try {
            await createBulkPart1s(dataWithExamId);
            setLoading(false);
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const response = await axios.get(`/api/part1?examId=${examId}`);
        setArrPart1(response.data.arr); // Cập nhật state với danh sách câu hỏi
      } catch (error) {

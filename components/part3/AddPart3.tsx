...
const formSchema = z.object({
  ...
  questionText: z
    .string()
    .min(1, { message: "must be atleast 3 characters long." }),
  answer1: z.string().min(1, { message: "must be atleast 3 characters long." }),
  answer2: z.string().min(1, { message: "must be atleast 3 characters long." }),
  answer3: z.string().min(1, { message: "must be atleast 3 characters long." }),
  answer4: z.string().min(1, { message: "must be atleast 3 characters long." }),
  topicId: z.string().nullable(), // Change this to a single string,
  correctAnswer: z
    .string()
    .min(1, { message: "must be atleast 3 characters long." }), // Đáp án đúng
  explainAnswer: z
    .string()
    .min(3, { message: "must be atleast 3 characters long." }),
});

const AddPart3 = ({ exam, part3, handleDialogueOpen }: AddPart3FormProps) => {
  const [image, setImage] = useState<string | undefined>(part3?.imageFile);

  const [imageIsDeleting, setImageIsDeleting] = useState(false);

  const [audio, setAudio] = useState<string | undefined>(part3?.audioFile);
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        const response = await apiClient.get(USER_API_ROUTES.GET_TOPICS_PART3);
  ...
  const handleAudioDelete = (audio: string) => {
    ...
    const audioKey = audio.substring(audio.lastIndexOf("/") + 3);
  ...
  const handleQuestionDelete = (part3: QuestionPart3) => {
    setIsLoading(true);
    axios
      .delete(`/api/part3/${part3.id}`)
      .then(() => {
        router.refresh();
        toast({
          variant: "default",
  ...
  const handleOpenDialogue = () => {
    // Assuming you want to open the dialogue for "part3"
    handleDialogueOpen("part3");
  };
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (exam && part3) {
      axios
        .patch(`/api/part3/${part3.id}`, values)
        .then((res) => {
          toast({
            variant: "default",
      ...
      axios
        ...
        .post("/api/part3", { ...values, examId: exam.id })
        .then((res) => {
          toast({
            variant: "default",
          ...
          <div className="pt-4 pb-2 flex gap-2">
            {part3 ? (
              <>
                ...
                <Button
                  onClick={() => handleQuestionDelete(part3)}
                  type="button"
